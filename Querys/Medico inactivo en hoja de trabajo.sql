...
-----------------------------------Hoja de Trabajo
select GENMEDICO1, * from SLNSERPRO where ADNINGRES1 IN (select OID from ADNINGRESO where AINCONSEC in (255138))
and GENMEDICO1 in (select OID from GENMEDICO where GMEESTADO <> 0)
-----------------------------------Cirugias
select GENMEDICO1, * from SLNPAQHOJ where SLNSERHOJ1 in (select OID from SLNSERPRO where ADNINGRES1 IN (select OID from ADNINGRESO where AINCONSEC in (255138)))
and GENMEDICO1 in (select OID from GENMEDICO where GMEESTADO <> 0)
---------------------------------------------------------------------------------------------
...
group by GENMEDICO1)
---------------------------------------------------------------------------------------------
select * from slnserhoj where oid=3926245
select * from slnserpro where oid=3926245
---------------------------------------------------------------------------------------------